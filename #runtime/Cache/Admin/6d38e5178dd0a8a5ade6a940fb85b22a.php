      ...
      <tbody>
        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$r): $mod = ($i % 2 );++$i;?><tr>
          ...
          <?php  echo $date_time = substr($r['date_time'],0,strlen($r['date_time'])-8); ?>
           ...
           <td align='center'>
            <?php echo ($r["s_price"]); ?>-<?php echo ($r["e_price"]); ?>
           </td>
          <td align='center' >
          <a href="<?php echo U("Products/productspreceup",array("id"=>$r['id']));?>">编辑</a> |
           <a class="J_ajax_del" href="<?php echo U("Products/productsprecedel",array("id"=>$r['id']));?>">删除</a>
           </td>
