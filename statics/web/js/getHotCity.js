...
function load_domestic_cities_data(){
	...
	if (typeof (station_names) != "undefined") {
		...
		liarray_cities4 = [];
		liarray_cities5 = [];
		var a_list = [], b_list = [], c_list = [], d_list = [], e_list = [], f_list = [], g_list = [], h_list = [], i_list = [], j_list = [], k_list = [], l_list = [], m_list = [], n_list = [], p_list = [], q_list = [], r_list = [], s_list = [], t_list = [], u_list = [], v_list = [], w_list = [], x_list = [], y_list = [], z_list = [];
		// 分拆城市信息
		var cities = station_names.split('@');
		for ( var i = 0; i < cities.length; i++) {
			var titem = cities[i];
			...
			if (titem.length > 0) {
				...
				if (favcityID != "" && titem[2] == favcityID) {
					favcity = titem;
					cities_all_in.unshift(titem);
					// 当fav城市位于第一页时，避免重复显示
					if (i > 6) {
						cities_all_in.push(titem);
					}
				} else {
					cities_all_in.push(titem);
				}
		...
		for ( var i = 0; i < cities_all_in.length; i++) {
			cities_all_in[i].push(i);
		}
		...
		liarray_cities4.push(p_list);
		liarray_cities4.push(q_list);
		liarray_cities4.push(r_list);
		liarray_cities4.push(s_list);
		liarray_cities4.push(t_list);
		liarray_cities4.push(u_list);
		liarray_cities4.push(v_list);
		liarray_cities4.push(w_list);
		
		liarray_cities5.push(x_list);
		liarray_cities5.push(y_list);
		liarray_cities5.push(z_list);
	}
	/*-----------------------------------------------------------------加载国内热门城市数据---------------------------------------------------------------------------*/
	if (typeof (favorite_names) != "undefined") {
		cities_fav_in = [];	
...
function load_international_cities_data(){
	cities_all_out = [];
	/*-----------------------------------------------------------------加载国际站点城市数据---------------------------------------------------------------------------*/
	if (typeof (favorite_abroad_names_out) != "undefined") {
		ularray_cities_showing0 = [];// 显示中的城市
		// 分拆城市信息
		var cities = favorite_abroad_names_out.split('@');
		for ( var i = 0; i < cities.length; i++) {
			var titem = cities[i];
			if ($.trim(titem) != '') {
				ularray_cities_showing0.push(titem.split('|'));
			}
			...
			if (titem.length > 0) {
				...
				if (favcityID != "" && titem[2] == favcityID) {
					favcity = titem;
					cities_all_out.unshift(titem);
					// 当fav城市位于第一页时，避免重复显示
					if (i > 6) {
						cities_all_out.push(titem);
					}
				} else {
					cities_all_out.push(titem);
				}
	...
	if (typeof (station_abroad_names_asia) != "undefined") {
		ularray_cities_showing1 = [];// 显示中的城市
		// 分拆城市信息
		var cities = station_abroad_names_asia.split('@');
		for ( var i = 0; i < cities.length; i++) {
			var titem = cities[i];
			if ($.trim(titem) != '') {
				ularray_cities_showing1.push(titem.split('|'));
			}
	...
	if (typeof (station_abroad_names_european) != "undefined") {
		ularray_cities_showing2 = [];// 显示中的城市
		// 分拆城市信息
		var cities = station_abroad_names_european.split('@');
		for ( var i = 0; i < cities.length; i++) {
			var titem = cities[i];
			if ($.trim(titem) != '') {
				ularray_cities_showing2.push(titem.split('|'));
			}
	...
	if (typeof (station_abroad_names_americas) != "undefined") {
		ularray_cities_showing3 = [];// 显示中的城市
		// 分拆城市信息
		var cities = station_abroad_names_americas.split('@');
		for ( var i = 0; i < cities.length; i++) {
			var titem = cities[i];
			if ($.trim(titem) != '') {
				ularray_cities_showing3.push(titem.split('|'));
			}
	...
	if (typeof (station_abroad_names_africa) != "undefined") {
		ularray_cities_showing4 = [];// 显示中的城市
		// 分拆城市信息
		var cities = station_abroad_names_africa.split('@');
		for ( var i = 0; i < cities.length; i++) {
			var titem = cities[i];
			if ($.trim(titem) != '') {
				ularray_cities_showing4.push(titem.split('|'));
			}
	...
	if (typeof (station_abroad_names_oceania) != "undefined") {
		ularray_cities_showing5 = [];// 显示中的城市
		// 分拆城市信息
		var cities = station_abroad_names_oceania.split('@');
		for ( var i = 0; i < cities.length; i++) {
			var titem = cities[i];
			if ($.trim(titem) != '') {
				ularray_cities_showing5.push(titem.split('|'));
			}
			if (titem.length > 0) {
				titem = titem.split('|');
				if (favcityID != "" && titem[2] == favcityID) {
	...
	for ( var i = 0; i < cities_all_out.length; i++) {
		cities_all_out[i].push(i);
	}
	/*-----------------------------------------------------------------加载国内热门城市数据---------------------------------------------------------------------------*/
	if (typeof (favorite_abroad_names_in) != "undefined") {
		cities_fav_out = [];	
		// 分拆城市信息
		var favcities = favorite_abroad_names_in.split('@');
		for ( var i = 0; i < favcities.length; i++) {
			var titem = favcities[i];
			if (titem.length > 0) {
				titem = titem.split('|');
				cities_fav_out.push(titem);
			}
		}		
		for ( var i = 0; i < fav_cities.length; i++) {
			cities_fav_out[i].push(i);
		}
	}
}
	...
	for (i = 1; i <= count; i++) {
		if (i == el) {
			...
			if (i > 1) {
				if (type == 0) {
					// 获取有序的城市集合
					var length = getCityName(type, i - 1, -1);
					for ( var k = 0; k < length; k++) {
						var list = getCityName(type, i - 1, k);
						if (list.length > 0) {
							ulHtml += '<li style="width: 45px;color: orange;text-align: center;cursor: auto;border:1px #fff solid;">' + list[0][0].toString().charAt(0).toUpperCase() + '</li>';
						}
						for ( var b = 0; b < list.length; b++) {
							if (b != 0 && b % 4 == 0) {
								ulHtml += '<li style="width: 45px;cursor: auto;border:1px #fff solid;"></li>';
							}
							ulHtml += '<li class="ac_even" style="border:1px #fff solid;" onClick="javascript:evenClick($(this));" title="' + list[b][1] + '" data="' + list[b][2] + '">' + list[b][1] + '</li>';
						}
						if (list.length % 4 != 0) {
							for (var b = list.length % 4; b < 4; b++) {
								ulHtml += '<li class="ac_even" style="cursor: auto;border:1px #fff solid;"></li>';
							}
						}
						$("#ul_list" + i).html(ulHtml);
					}
				} else {
					var list = getCityName(type, i - 1, -2);
					for ( var k = 0; k < list.length; k++) {
						ulHtml += '<li class="ac_even" style="border:1px #fff solid;" onClick="javascript:evenClick($(this));" title="' + list[k][1] + '" data="' + list[k][5] + '">' + list[k][1] + '</li>';
					}
					$("#ul_list" + i).html(ulHtml);
				}
			}
	...
	switch (type) {
		...
		case 1:
			switch (nod) {
				...
				case 1:
					if (at == -1) {
						return ularray_cities_showing0.length;
					}
					if (at == -2) {
						return ularray_cities_showing0;
					}
					return ularray_cities_showing0[at];
					break;
				case 2:
					if (at == -1) {
						return ularray_cities_showing1.length;
					}
					if (at == -2) {
						return ularray_cities_showing1;
					}
					return ularray_cities_showing1[at];
					break;
				case 3:
					if (at == -1) {
						return ularray_cities_showing2.length;
					}
					if (at == -2) {
						return ularray_cities_showing2;
					}
					return ularray_cities_showing2[at];
					break;
				case 4:
					if (at == -1) {
						return ularray_cities_showing3.length;
					}
					if (at == -2) {
						return ularray_cities_showing3;
					}
					return ularray_cities_showing3[at];
					break;
				case 5:
					if (at == -1) {
						return ularray_cities_showing4.length;
					}
					if (at == -2) {
						return ularray_cities_showing4;
					}
					return ularray_cities_showing4[at];
					break;
				case 6:
					if (at == -1) {
						return ularray_cities_showing5.length;
					}
					if (at == -2) {
						return ularray_cities_showing5;
					}
...
//显示国际城市
function showInternationalCity(){
	var tHtml = "";
	tHtml = '<div class="com_hotresults" id="thetable" style="width:436px">'
			...
			+ '<li class="action" index="1" method="liHotTab"  onclick="jsDomestic(1, 1)" id="nav_list1" style="width: 60px;">国内热门</li>'
			+ '<li index="2" method="liHotTab"  onclick="jsDomestic(2, 1)" id="nav_list2" style="width: 60px;">国际热门</li>'
			+ '<li index="3" method="liHotTab"  onclick="jsDomestic(3, 1)" id="nav_list3" style="width: 30px;">亚洲</li>'
			+ '<li index="4" method="liHotTab"  onclick="jsDomestic(4, 1)" id="nav_list4" style="width: 30px;">欧洲</li>'
			+ '<li index="5" method="liHotTab"  onclick="jsDomestic(5, 1)" id="nav_list5" style="width: 30px;">美洲</li>'
			+ '<li index="6" method="liHotTab"  onclick="jsDomestic(6, 1)" id="nav_list6" style="width: 30px;">非洲</li>'
			+ '<li index="7" method="liHotTab"  onclick="jsDomestic(7, 1)" id="nav_list7" style="width: 40px;">大洋洲</li>'
			+ '</ul>'
			+ '<ul class="popcitylist" style="overflow: hidden;padding-bottom: 10px;padding-left: 30px;" method="hotData" id="ul_list1">';

	var favTotelLength = getCityName(1, 0, -1);
	for ( var b = 0; b < favTotelLength; b++) {
		tHtml += '<li class="ac_even" title="' + getCityName(1, 0, b)[1] + '" data="' + getCityName(1, 0, b)[5] + '">' + getCityName(1, 0, b)[1] + '</li>';
	}
	tHtml += '</ul>';

	for ( var a = 2; a <= 7; a++) {
		tHtml += '<ul  class="popcitylist" style="overflow: hidden;display:none;padding-left: 30px;" id="ul_list' + a + '">';
		tHtml += '</ul>';
	}
...
function showDomesticCity(){
	...
	tHtml = '<div class="com_hotresults" id="thetable" style="width:436px">'
			...
			+ '<ul class="AbcSearch clx" id="abc">'
			+ '<li class="action" index="1" method="liHotTab"  onclick="jsDomestic(1, 0)" id="nav_list1">热门城市</li>'
			+ '<li index="2" method="liHotTab"  onclick="jsDomestic(2, 0)" id="nav_list2" style="width: 60px;">ABCDEF</li>'
			+ '<li index="3" method="liHotTab"  onclick="jsDomestic(3, 0)" id="nav_list3" style="width: 40px;">GHIJ</li>'
			+ '<li index="4" method="liHotTab"  onclick="jsDomestic(4, 0)" id="nav_list4" style="width: 40px;">KLMN</li>'
			+ '<li index="5" method="liHotTab"  onclick="jsDomestic(5, 0)" id="nav_list5" style="width: 80px;">PQRSTUVW</li>'
			+ '<li index="6" method="liHotTab"  onclick="jsDomestic(6, 0)" id="nav_list6" style="width: 30px;">XYZ</li>'
			+ '</ul>'
			+ '<ul class="popcitylist" style="overflow: hidden;padding-bottom: 10px;padding-left: 30px;" method="hotData" id="ul_list1">';

	var favTotelLength = getCityName(0, 0, -1);
	for ( var b = 0; b < favTotelLength; b++) {
		tHtml += '<li class="ac_even" title="' + getCityName(0, 0, b)[1] + '" data="' + getCityName(0, 0, b)[3] + '">' + getCityName(0, 0, b)[1] + '</li>';
	}
	tHtml += '</ul>';

	for ( var a = 2; a <= 6; a++) {
		tHtml += '<ul  class="popcitylist" style="overflow: hidden;display:none;" id="ul_list' + a + '">';
		tHtml += '</ul>';
	}
...
$(document).ready(
		function() {
			...
			// 初始化国际城市数据
			load_international_cities_data();
			// showAllCity();
			isClick = false;
			fixDivBugInIE6($('#form_cities'));
			fixDivBugInIE6($('#form_cities2'));
			
			$("#fromCityCn").focus(function() {
				if ($("input[name='FlightType']:checked").val() == '20'){
					// 空条件过滤出国内城市列表
					showDomesticCity();
				} else {
					// 空条件过滤出国际城市列表
			...
			// 入住城市		
			$("#inCity").focus(function() {
				if ($("input[name='hotelType']:checked").val() == '10'){
					// 空条件过滤出国内城市列表
					showDomesticCity();
				} else {
					// 空条件过滤出国际城市列表
