    ...
    public function manager() {
        ...
        if($roleid != 1){
            ...
            while (!empty($role_queue)) {
                ...
                $where['pid'] = $role;
                $result = $Role->where($where)->select();
        ...
        if (empty($role_id)) {
            ...
            $count = $UserView->where($where)->count();
            $page = $this->page($count, 20);
            $User = $UserView->where($where)->limit($page->firstRow . ',' . $page->listRows)->select();
        } else {
            if($roleid != 1 && !in_array($role_id, $role_list)){
                $this->error("您没有权限查看此角色");
            }
