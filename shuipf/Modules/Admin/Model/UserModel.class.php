...
class UserModel extends RelationModel {
    ...
    public function delUser($userId) {
        ...
        if (false !== $this->where(array('id' => $userId))->delete()) {
            //搜索并删除该用户的所有子角色
            $Role = D("Role");
            $result =  $Role->where(array("create_user_id" => $userId))->select();
            foreach ($result as $key => $value) {
                $Role->delete_role($value['id']);
            }
            //不需要删除这个了
            // //删除此用户创建的文章，礼品，活动
            // $Daynews=M("Daynews");
            // $Daynews->where(array('create_user_id'=>$userId))->delete();

            // $activities=M("Activities");
            // $activities->where(array('create_user_id'=>$userId))->delete();

            // $Products=M("Products");
            // $Products->where(array('create_user_id'=>$userId))->delete();
            return true;
        } else {
            $this->error = '删除失败！';
            return false;
